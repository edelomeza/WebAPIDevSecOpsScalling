...
| # | Entorno | Paso | Descripción | Costo | Estado | DescripcionDev |
|---|---|---|---|---|---|---|
| 4.0 | **GitHub** | DB externa `ExternalDbHost` | Consolidación 2.1B+3.3B: `GH Variables RDS_ADDRESS=db45497.public.databaseasp.net, DB_NAME=db45497` (ex `188.40.211.8` Hetzner) + `CFN ExternalDbHost param / CreateRDS: !Equals [ExternalDbHost,""] / ExternalDbHostUsed output` → `compose Server=${RDS_ADDRESS}` `Database=${DB_NAME:-db45497}` `Encrypt=True;TrustServerCertificate=True` — si `ExternalDbHost≠""` evita crear `RDS db.t3.micro` | $0 | ✅ Concluido | Migra Hetzner→Monster public: `public.databaseasp.net` `Encrypt` `TrustServerCertificate` para TLS Internet AWS→Monster; `GH vars` `RDS_ADDRESS` `public` `2026-09-18T18:49` reemplaza Hetzner `whitelist 0.0.0.0/0` |
| 4.1 | **AWS** | `bash deploy-aws.sh webapidevsecops-prod` | **Option A:** `CREATE_COMPLETE` 8-12m (RDS lento). **Option B:** `CREATE_COMPLETE` 3-5m sin RDS/DBSubnetGroup/SGRDS (`ExternalDbHost=db45497.public.databaseasp.net`), `parameter-overrides ExternalDbHost` sin `DBPassword` | **Inicia facturación** — ver cálculo abajo (B sin RDS) | ✅ Concluido | Crea infra base Only B en 3m: VPC+ALB+EC2 t3.micro AL2023 `ami-0bd3fbcdc633a1b1a` kernel-default-x86_64+SQS FIFO x4+Logs sin RDS; expone ALB `webapidevsecops-prod-alb-1055914754.us-east-1.elb.amazonaws.com` y EC2 `100.54.31.182` vía `ExternalDbHost=db45497.public.databaseasp.net/db45497`; `ImageTagUsed=9701a7b9dc15954d6cbba9d8db57576763253547`; `EC2 i-046788481b3efda33` `100.52.168.93→100.54.31.182` tras `ImageTag` `0a2d46d→9701a7b`; inicia facturación ALB ~$6.83/14d (16h MX) |
| 4.2 | **EC2** | `deploy-app.sh` | `docker pull edelomeza/webapidevsecops-scalling:${sha}` + `up -d` + migr `Program.cs:464`. **Option B:** `RDS_ADDRESS=$EXTERNAL_DB_HOST` priorizado `deploy-app.sh:33` sobre `describe-stacks Output RDSAddress/ExternalDbHostUsed` → `docker-compose.aws.yml:29` `ConnectionStrings__DefaultConnection` + `DB_USER/DB_PASSWORD` `Program.cs:117-128` `SqlConnectionStringBuilder` → `ctx.Database.Migrate():487` guard `SkipMigration` | $0 | ✅ Concluido | **Final public DB**: desplegado vía SSM con imagen trazable `9701a7b9dc15954d6cbba9d8db57576763253547` (`0a2d46d`→`9701a7b`), `AMI AL2023` `docker compose v5.5.1` `Success` `i-046788481b3efda33` `100.54.31.182` `SKIP_MIGRATION=true` `RDS_ADDRESS=db45497.public.databaseasp.net` `Encrypt=True;TrustServerCertificate=True`; `docker ps` api+redis up, `UnitTest 603 Passed` `CircuitBreaker 2s/2.5s` + `IntegrationTest HealthCheck Dispose null-guard` `357 Passed`; `GH vars` `public` reemplaza Hetzner |
| 4.3 | **AWS** | Verificar | `curl http://$ALB_DNS/health/ready` `200` (tag `db` `Program.cs:585` + `AddSqlServer:154`), SQS 4 `fifo` vacías luego con mensajes tras saga. **Option B:** `/health/ready` contra `db45497.public.databaseasp.net/db45497` | $0 | ✅ Concluido | **Verificado 200 OK** `curl http://webapidevsecops-prod-alb-1055914754.us-east-1.elb.amazonaws.com/health/ready` `{"status":"Healthy","entries":{"sql-server":{"status":"Healthy"}}}` `Deploy 35386250891` `health 200 OK intento 1` `SSM docker compose up Success` `i-046788481b3efda33` `100.54.31.182` `ImageTagUsed 9701a7b`; `SQS FIFO x4` `https://sqs.us-east-1.amazonaws.com/151105438268/...pedidos*.fifo` vacías; `UnitTest 603` `IntegrationTest 357` `0 Failed` tras fix `CircuitBreaker` + `Dispose` |
| 4.4 | **GitHub** | `schedule-destroy.yml` `01:00 MX` | `cron 0 7 * * * UTC` `America/Mexico_City UTC-6` → `aws cloudformation delete-stack webapidevsecops-prod` + `wait stack-delete-complete` diario 7/7. `schedule + workflow_dispatch`, `concurrency deploy-production false`, sin `environment: production` (bloquearía schedule) | `.github/workflows/schedule-destroy.yml` nuevo ~40l | $0 | ✅ Concluido | **Creado en `DespliegueCorteNocturno`**: `36l` `schedule-destroy.yml` `cron 0 7 UTC` `01:00 MX` `workflow_dispatch` `concurrency deploy-production` `runs-on ubuntu-latest` `checkout` `configure-aws-credentials` `secrets AWS_*` `vars RDS_ADDRESS public` `bash scripts/destroy-aws.sh`; `DeletionPolicy Delete` borra ALB+TG+EC2+SQSx4+LogGroup+VPC `3-5m` `B`; no borra `db45497.public.databaseasp.net`; `off 8h` `33%` `224h` |
| 4.5 | **GitHub** | `schedule-deploy.yml` `09:00 MX` | `cron 0 15 * * * UTC` `America/Mexico_City UTC-6` → `deploy-aws.sh` `CREATE_COMPLETE` 3-5m + `deploy-app.sh` `SSM docker pull/up` `RDS_ADDRESS=188.40.211.8` `SKIP_MIGRATION=true` + `curl /health/ready` | `.github/workflows/schedule-deploy.yml` nuevo ~65l | $0 | ✅ Concluido | **Creado en `DespliegueCorteNocturno`**: `61l` `schedule-deploy.yml` `cron 0 15 UTC` `09:00 MX` `workflow_dispatch` `concurrency deploy-production` `runs-on ubuntu-latest` `checkout` `configure-aws` `deploy-aws.sh "${{github.sha}}"` `RDS_ADDRESS public` `DB_NAME` `JWT` `SSM` `deploy-app.sh "${{github.sha}}"` `SKIP_MIGRATION true` `health/ready 200`; `8-10m` `ImageTag` `ALBDNS` nuevo diario |
