...
namespace SecurityTest.TipoEmpleado;
...
public class SecurityTests : IClassFixture<WebApplicationFactory<Program>>, IAsyncLifetime
{
    ...
    public SecurityTests(WebApplicationFactory<Program> factory)
    {
        _client = factory.WithWebHostBuilder(builder =>
        {
            ...
            builder.UseSetting("InMemoryDatabaseName", $"SecurityTestTipoDb_{Guid.NewGuid():N}");
        }).CreateClient();
    }
    ...
    {
        var getResponse = await _client.GetAsync("/api/v1/tipoempleado");
        Assert.Equal(HttpStatusCode.Unauthorized, getResponse.StatusCode);

        var getByIdResponse = await _client.GetAsync("/api/v1/tipoempleado/1");
    ...
    {
        var response = await _client.GetAsync("/api/v1/tipoempleado");
